...
layui.define(['code', 'element', 'table', 'util', 'carousel', 'laytpl'], function(exports){
  ...
  layer.ready(function(){
    //升级提示
    if(local.version){
      var vLocal = local.version.split('.')
      ,vNow = layui.v.split('.') 
      ,updatable;
      ...
      layer.open({
        ...
        ,yes: function(index){
          layer.close(index);
          setTimeout(function(){
            location.href = (url.pathname[0] === 'v2' ? '/v2' : '') 
  ...
  ;!function(){
    var elemComponentSelect = $(['<select lay-search lay-filter="component">'
      ...
      ,'<option value="https://'+ home.devHost +'/products/layim/">layim</option>'
      ,'<option value="https://'+ home.devHost +'/products/layuiAdmin/">layuiadmin</option>'
    ,'</select>'
